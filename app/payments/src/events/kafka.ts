...
const kafka_init = async () => {
  if (!process.env.CLIENT_ID) throw new Error('[Payments] Client-ID must be defined')
  if (!process.env.BROKERS) throw new Error("[Payments] Can't find a list of brokers")
